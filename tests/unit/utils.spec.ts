...
describe("fetchData function", () => {
  it("should return json when response status is 200", async () => {
    const expected = { test: 1 };
    const json = jest.fn().mockResolvedValueOnce(expected);
    const fetch = jest.fn().mockResolvedValueOnce({ status: 200, json });
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://yet-another-covid-api.herokuapp.com/cases?from=4/26/20&to=4/27/20&country=&aggregatecountries=false&perday=false&worldtotal=true"
    );
    expect(result).toEqual(expected);
  });
  it("should return null when response status is not 200", async () => {
    const json = jest.fn().mockResolvedValueOnce({ test: 1 });
    const fetch = jest.fn().mockResolvedValueOnce({ status: 404, json });
    const result = await fetchData(
      Endpoints.NEWS,
      "4/26/20",
      "4/27/20",
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://yet-another-covid-api.herokuapp.com/news?from=4/26/20&to=4/27/20&country=Singapore&aggregatecountries=false&perday=false&worldtotal=false"
    );
